...
let assistData = {};                //輔助平台data model
let _selZMAKE = "";                 //廠代預設清單
let checkInsureExistResult = null;
let btnAssistCloseMsg = "此報價不成立，一旦離開，則報價時需要重新執行報價。";
/*====[初始化]====*/
window.onload = function () {
    console.log('檢查輔助來源資料');
    console.log($('#hidAssistDataJson').val());
    let hasAssistData = true;
    try {
        assistData = JSON.parse($('#hidAssistDataJson').val());
    } catch (e) {
        hasAssistData = false;
    }
    if (!hasAssistData || (assistData == undefined || assistData == null)) {
    ...
    setTimeout(function () {
        ...
        setTimeout(function () {
            window.scrollTo(0, 0);//輔助平台預設將畫面停在最上方
            consLogDate("--==輔助平台預設將畫面停在最上方==--");
        }, 3000);
    }, 1);
}

//廠代選項初始化
function InitZMAKEList(brandModel, insuredCarType)
{
    let ZMAKEListResult = getdata("/Assist/GetAssistZMAKEList", { MakeCD: brandModel, CarType: insuredCarType });
    if (ZMAKEListResult.Success) {
        _arrZMAKEList = ZMAKEListResult.Data;
        for (let i = 0; i < _arrZMAKEList.length; i++) {
            if (_arrZMAKEList[i].TEXT.trim() != "") {
                _selZMAKE += '<option value="' + _arrZMAKEList[i].VALUE + '">' + _arrZMAKEList[i].VALUE + ' - ' + _arrZMAKEList[i].TEXT + '</option>';
            };
        };
        $("#selZMAKE").empty().append('<option value="">請選擇</option>' + _selZMAKE).selectpicker('refresh');
    }
}
...
function InitComp() {
    ...
    if (((new Date()).getMonth() + 1) < 11) {
        //[年份]大於等於11月時，才出現明年度的選項可選(目前月份<11月時，則顯示今年度就好)
    ...
    if (sessionStorage["dtSys"] == undefined)
        sessionStorage["dtSys"] = JSON.stringify(getdata("/Quotation/GetLoadSysData", { sProgCode: 'ASSIST' }));                   //查詢[系統載入預設值]相關資訊
    ...
    setDDL_SYSCODEDT(dtSys, 'ASSISTSOLU', '#selZCAMPAN');                          //[活動方案]
...
function InitEvent() {
            ...
            case "chkMan":      /*----強制險--------------------------*/
                if (chkMan.prop('checked')) {
                    ...
                    if ($("#iptFRDATES").val() == '' && $("#iptFRDATEE").val() == '') { //若保期[起日]有值，則更新[迄日]
                        ...
                        $('#iptFRDATEE').val(AddYears($("#iptFRDATES").val(), 1)).blur().change();
                    }
                } else {
                    //[強制車種]清空
    ...
    _div3.find("#selZMAKE").SetKeyinFunction({
        ...
        func: function (e, select, title) {
            ...
            //let data = _arrZMAKEList.filter(function (x) { return x.VALUE.indexOf(e.val()) > -1 });
            //var OptionHtml = '';
            //if (data.length > 0) {
...
//*
            ...
            if (seldata.length > 0) {
                ...
                //搜尋不到時清除暫存值
                BindSelpikTabEvent.spkVal = '';
                selObj.append(_selZMAKE).selectpicker('refresh');
            }
    ...
    _div3.find("#selZMAKE").on("change", function () {
        ...
        iptRESETPRICE.val('');
        if (sVal == "") { _div3.find('#selMVPZMTYPE').val(''); return; }
        //解析[廠牌名稱]取得CC數及乘載
        ...
        //let arRetStr = sRetStr.split('|');
        //if (arRetStr != null && arRetStr.length > 1) {
        //    if (arRetStr[0] != "")
        //        _div3.find('#iptZCC').val(arRetStr[0]);       /*CC數*/
        //    if (arRetStr[1] != "")
        //        _div3.find('#iptZCARRY').val(arRetStr[1]);    /*承載*/
        //}
        //讀取[廠型]相關資訊
        ...
        if (dt.length > 0) {
            ...
            _div3.find('#selMVPZMTYPE').prepend("<option value=''></option>").val('');                 /*[車種]預設空白*/
            ...
            //}
            //輔助平台帶入廠代名稱
            setSelPick("#selZMAKE", [{ VALUE: sVal, TEXT: dt[0].CARDESC }], null);    //廠型
        }
        //[CC數]特別處理
    ...
    _div1.find("#selAGNTNUM").on("change", function () {
        ...
        $.each(data, function (index, data) {
            _div1.find('#iptBranchNo').val(getObjToVal(data.Branch));    //單位別
        ...
        if (dt.length > 0) {
            ...
            //--登入者為[收費員]特別處理_E-------------
            _div1.find('#iptCNTBRANCH').val(dt[0].LifeNo);                                 //業務人員編號
    ...
    $('#chkISAPL').on("change", function () {
        var divAPL = $('#divAPLData');
        var divIns = $('#divInsurerData');
        if (this.checked) {
            divAPL.find("#rdoAPLCustType[value='" + $('#rdoCTLCustType:checked').val() + "']").prop("checked", true).change();
            setDDLIsNullAdd('#selAPLNation', divIns.find('#selCTLNation').val());                   //國別
            divAPL.find('#iptAPLCustID').val(divIns.find('#iptCTLCustID').val());                   //ID/公司統編
            divAPL.find('#iptAPL_CLNTNUM').val(divIns.find('#iptCTL_CLNTNUM').val());               //客戶代號
            divAPL.find('#iptAPLCustID').data("Is70BNO", divIns.find('#iptCTLCustID').data("Is70BNO"));     //是否有70單位別
            divAPL.find('#iptAPLCustID').data("IsPEND", divIns.find('#iptCTLCustID').data("IsPEND"));       //是否有未決賠案
            divAPL.find('#iptAPLRepresentative').val(divIns.find('#iptCTLRepresentative').val());   //代表人
            divAPL.find('#iptAPLName').val(divIns.find('#iptCTLName').val());                       //姓名/公司名稱        
            divAPL.find('#iptAPLBirthday').val(divIns.find('#iptCTLBirthday').val()).blur();        //生日
            divAPL.find('#selAPLSex').val(divIns.find('#selCTLSex').val());                         //性別
            divAPL.find('#selAPLMarriage').val(divIns.find('#selCTLMarriage').val());               //婚姻
            divAPL.find('#iptAPLOfficeTel').val(divIns.find('#iptCTLOfficeTel').val());             //公司電話
            divAPL.find('#iptAPLHomeTel').val(divIns.find('#iptCTLHomeTel').val());                 //家用電話
            divAPL.find('#iptAPLCellPhone').val(divIns.find('#iptCTLCellPhone').val());             //手機號碼
            divAPL.find('#iptAPLFax').val(divIns.find('#iptCTLFax').val());                         //傳真號碼
            divAPL.find('#selAPLADD1').val(divIns.find('#selCTLADD1').val()).change();              //地址1
            divAPL.find('#selAPLADD2').val(divIns.find('#selCTLADD2').val());                       //地址2
            divAPL.find('#iptAPLADDO').val(divIns.find('#iptCTLADDO').val());                       //地址3
            divAPL.find('#iptAPLADDPot').val(divIns.find('#iptCTLADDPot').val());                   //地址(郵遞區號)
            divAPL.find('#iptAPLEmail').val(divIns.find('#iptCTLEmail').val());                     //E-Mail
            divAPL.find('#selRELA').val('01');    //與被保險人關係固定為01
            divAPL.hide();    //隱藏[要保險人資料]區
        } else {
            divAPL.show();    //顯示[要保險人資料]區
            divAPL.find('input[type=text],input[type=tel],select').each(function (i, item) {
                $(item).val('');        //清空資料
                if (item.id == "selAPLADD1") {              //[地址1]執行變更事件
                    $(item).change();
                } else if (item.id == "selAPLNation") {     //[國別]預設值為TWN
                    $(item).val('TWN');
                }
            });
            divAPL.find("#rdoAPLCustType[value='P']").prop("checked", true).change();   //[客戶類別]預設為"個人"
            //divAPL.find("#btnUnAPLCustID").click(); //[解鎖]按鈕(被保險人)
        }
    });
    //[牌照]焦點離開事件
    ...
    //[製造年]選取事件
    _div3.find('#selYRMANF').on("change", function (e) {
        var sVal = this.value;
        _div3.find('#selIssueYear').val(sVal);
        var sZMAKE = _div3.find("#selZMAKE").val();
        if (sVal != '' && sZMAKE != '') {
            var iptRESETPRICE = _div3.find('#iptRESETPRICE');
            iptRESETPRICE.val('');
            //讀取[重置價格]
            var dt = getdata("/Quotation/GetRESETPRICE", { sMakeCD: sZMAKE, sYEAR: sVal });
            if (dt.length > 0) {
                if (dt.length === 1 && typeof (dt[0].MSG) != "undefined") { MsgBox('錯誤', dt[0].MSG, 'red'); return; }
                iptRESETPRICE.attr('oldval', getObjToVal(dt[0].LIST)).val(getObjToVal(dt[0].LIST)).blur();
                iptRESETPRICE.data('oldval', getObjToVal(dt[0].LIST));
    ...
    $('#selMVPZMTYPE').on("change", function (e) {
        ...
        var sVal = this.value;
        if (sVal == "") { 
            _div4.find('#chkAny').prop('checked', false).parent().next().next('.panel-info').hide(200); return;
        }
        var dt = [];
        //承載單位(P-人,T-噸)
        dt = _arrLoadSysData.filter(function (x) { return x.CodeNo == 'T002_1' && x.VALUE == sVal; });
    ...
    //[險種資料]相關欄位_事件([任意車種],[廠型],[重置價格],[任意保期(起)],[任意保期(迄)],[乘載],[發照年月],[折舊率])
    /*內包含[取級數(任意險)]相關欄位_事件([任意車種],[任意保期(起)],[任意保期(迄)])*/
    $('#selMVPZMTYPE,#iptRESETPRICE,#iptCCDATES,#iptCCDATEE,#iptZCARRY,#selIssueYear,#selDepRate').on("change", function (e) {
    ...
    $('#chkEmail').on("change", function () {
        if (this.checked) {
            if ($('#iptAPLEmail').val() == '') {
                ctrlMsg('MSFO2', '選擇電子式保險證，要保人EMail地址必須輸入！', '#iptAPLEmail');
    ...
    //輔助平台調整頁面上若任意險保期(起、迄)、責係、體係、活動方案、任意車種有被異動，則確定投保按鈕將disable，待計算保費鈕有按下時，才會ON起來
    $('#iptCCDATES, #iptCCDATEE, #selBodyFactor, #selDutyFactor, #selZCAMPAN, #selMVPZMTYPE').change(function () {
...
function InitClick() {
    ...
    $('#btnCalPre').on("click", function (e) {
        ...
        setTimeout(function () {
            ...
            if (msg == "") {
                //代收機構時須檢核有無投保任意險，無:停用確定投保鈕
                if (assistData.collectionType == "1" && selZCAMPAN.val() == "") {
                    $('#btnSubmit').prop('disabled', true);
                    //MsgBox('', '有代收機構須投保任意險！', 'orange');
                }
                else {
    ...
    $('#btnSubmit').on("click", function (e) {
        ...
        setTimeout(function () {
            ...
            //險種資料
            var INS_Arr = $('#tblInsuranceList').DataTable().data().toArray();
            //INS_Arr.push({ EXCESS: "10", InsMonth: "000000000", MVPPREM: "000007737", SUMINA: "2003000", SUMINB: null, SUMINC: null, ZCVRTYPE: "11", ZCVRTYPENM: "竊盜損失險xxxx", ZFACTORA: "", ZFACTORB: "" });
            //INS_Arr.push({ EXCESS: "52", InsMonth: "000000000", MVPPREM: "000027717", SUMINA: "2003000", SUMINB: null, SUMINC: null, ZCVRTYPE: "0552", ZCVRTYPENM: "竊盜損失險", ZFACTORA: "", ZFACTORB: "" });
            ...
            var Other_Arr = getQuotOtherData();
            //作業型態(INS:新增,ALT:修改)
            var worktype = "INS";  /*(TEMP:要保資料輸入,EDIT:修改)*/
            //寫入參數
            var param = {
                sWorkType: worktype, sProgCode: "ASSISTEdit"
            ...
            if (isChk) {
                ...
                //else
                {
                    ...
                    if (QuotNo400 == undefined || QuotNo400 == "err") { QuotNo400 = ""; }
                    ...
                    if (dt.length > 0) {
                        ...
                        //回寫輔助平台資料
                        assistData.QuotNo = dt[0].QuotNo;
                        let saveAssistDataResult = getdata("/Assist/SaveAssistOtherData", { model: assistData });
                        if (!saveAssistDataResult.Success) {
                            MsgBox('儲存失敗', '儲存輔助平台資料失敗', 'red');
                            return;
                        }

                        if (_strZautclsMail != "")
                            MsgBox('', '任意險需經核保人員審核，請洽保險公司服務人員。', 'green');
                        else {
                            //列印
                            PrintAssist(true);
                            MsgBox('', '報價成功。<BR>400報價單號碼：[' + QuotNo400 + ']', 'green');
                        }
                    }
                }
    ...
    //[離開]按鈕
    $('#btnAssistClose').on("click", function () {
        if (btnAssistCloseMsg != "")
            MsgBox('', btnAssistCloseMsg, 'orange');
        $('input,select,textarea,button').each(function () { this.disabled = true });      //所有[輸入框]disabled
        $('.navbar-toggle').attr('disabled', false);    //因為上述語法會將[選單]按鈕失效，所以再把按鈕打開
    ...
    function GetMinDate(date1, date2) {
        ...
        else if (date1 < date2) return date1;
        else return date1;
    }
    //檢核_[列印]
    function PrecheckPrint(sPrintType, sActionType) {
        let quotno = checkInsureExistResult.Data[0]["QuotNo"];
        let QuotNo_400 = checkInsureExistResult.Data[0]["QuotNo_400"];
        let AgentNo = checkInsureExistResult.Data[0]["AgentNo"];
        let dt1 = $('#iptFRDATES').val();
        let dt2 = $('#iptCCDATES').val();
        let InsSDATE = '';
        if (dt1 != '' && dt2 != '') {
            let current_datetime = GetMinDate(new Date(dt1), new Date(dt2))
            InsSDATE = current_datetime.getFullYear().toString() + ('0' + (current_datetime.getMonth() + 1)).substr(-2, 2) + ('0' + (current_datetime.getDate())).substr(-2, 2);
        }
        else if (dt1 != '' && dt2 == '')
            InsSDATE = dt1;
        else if (dt1 == '' && dt2 != '')
    ...
    //輔助平台列印
    function PrintAssist(isSendMail)
    {
        //初始化列印資訊
        let quotno = $("#iptQUOTENO").val();
        let userid = $('#hiduserid').val();
        let QuotNo_400 = $("#divQuoteDate").data("QuotNo_400");
        let arAgentNo = assistData.handledID.split('-');
        let AgentNo = arAgentNo != undefined && arAgentNo.length > 0 ? arAgentNo[1] : '';
        //let InsSDATE = $("#divQuery").data("InsSDATE");

        if (isSendMail) {
            let resultmailmodel = {};
            resultmailmodel["BranchNo"] = $('#iptBranchNo').val();
            resultmailmodel["AgentNo"] = AgentNo;
            resultmailmodel["QuotNo_400"] = QuotNo_400;
            resultmailmodel["UserName"] = assistData.producerName;
            resultmailmodel["AgentEmail"] = $('#iptAgentEMail').val();
            //發送信件至經手人的內部服務人員
            let sendAssistResult = getdata("/Assist/SendAssistResult", { mailModel: resultmailmodel });
        }
        ...
        let printItem = []; //列印項目 要保書/報價單:0, 強制險要保書:5, 繳款單:2
        //代收機構提供列印任意險要保書/報價單及繳款單 
        if (assistData.collectionType == "1") {
            printItem.push("0");
            printItem.push("2");
        }
        else {
            if ($('#selZCAMPAN').val() != "")
                printItem.push("0");
            //非代收機構提供列印任意險要保書報價單、強制險要保書及繳款單
            //險種資料
            var INS_Arr = $('#tblInsuranceList').DataTable().data().toArray();
            let ans = INS_Arr.some(function (item, index, array) {
                return item.ZCVRTYPE == "21" // 當險種有買21 強制險時需列印強制險要保書
            });
            if (ans) {
                printItem.push("5");
            }
        ...
        PrintReport("/Quotation/GenPDF", printItemModel, printItem, false);
    }
    ...
    //        var isChk = chkSub_SaleData();                  //檢核_{經手人/業務員資料}區塊資料
    ...
    //        }
    //        //AS400檢核
    //        if (isChk) { isChk = !(mqQuotation('1', '2') == "err"); }
    //        //檢核通過
    ...
    //            }
    //        }
    //        setDDLZCAMPAN(getdata("/Quotation/getUserInsList", {}));    //設定[活動方案/自訂方案]下拉選單內容
    //        //[續保作業]時，[折舊率]不可修改
...
function InitDefault() {
    ...
    //        if (getObjToVal(dt[0].TOLLCLTERID) != '') { //登入者為電銷人員，則打開[服務人員][電話][傳真][EMail]欄位(登入時會用ID去T9982找資料，若有，則為電銷人員)
    //            _div1.find('#iptAgentTEL,#iptAgentFAX,#iptAgentEMail').attr('disabled', false);
    //        }
    //    }
    //}
    ...
    switch (_strMode) {
        ...
        //    $('#btnQryQUOTENO,#btnStorCache,#btnSubmit,#btnClose').parent().hide();
        //    break;
        case 'AssistEDIT':            /*--[修改]模式--*/
            consLogDate("InitDefault_載入處理設定[修改]模式");
            $('#iptQUOTENO').attr('disabled', true);                    //鎖住[報價單號碼]
            getWebAssistData();                                         //讀取[輔助平台資料]
            $('#btnQryQUOTENO').parent().hide();                        //隱藏[複製報價單]按鈕
            $('#btnStorCache').parent().hide();                         //隱藏[暫存]按鈕
            //$('#btnSubmit').parent().addClass('col-lg-offset-3')        //調整[確定]按鈕位置
            $('#spanTitle').text("輔助平台");                         //調整標題
            insuredDT()                                                 //險種資料相關資料處理
            break;
        default:
    ...
    ctrlDiv(-1, "LOCK");            /*鎖住所有控制項*/
    if (_strMode == 'AssistEDIT') {
        consLogDate("InitDefault()_載入處理設定[檢視]模式(開啟所有頁)");
        //輔助平台UI控制_Start==================================================
        ctrlDiv(4, "EDIT");         //開啟險種區
        //輔助投保強制險對應頁面勾選項
        $('#chkMan').prop('disabled', true).prop('checked', assistData.compInsOrNot == "Y").change().parent().next().next('.panel-info').show(200);
        //投保單強時需鎖住任意險
        $('#selMANMVPZMTYPE').prop('disabled', true);
        //if (assistData.compInsOrNot == "Y" && assistData.arbtyInsOrNot == "N") {
        //    $('#selMVPZMTYPE').prop('disabled', true);
        //}
        if (assistData.arbtyInsOrNot == "Y") {
            $('#selMVPZMTYPE').prop('disabled', false);
        }
        else {
            $('#selMVPZMTYPE').prop('disabled', true);
            $('#chkAny').prop('checked', false).prop('disabled', true).change();
        }


        //代收時一律不提供強制險
        if (assistData.collectionType == "1")
            $('#chkMan').prop('checked', false).change().parent().next().next('.panel-info').show(200);

        $('#btnNext4').hide();
        //被保人區
        //if (assistData.idType == "2") {
        //    $('#iptCTLRepresentative').prop('disabled', false);     //被保人負責人解鎖
        //}
        //被保人家裡/公司/手機電話號碼
        //$('#iptCTLOfficeTel,#iptCTLHomeTel,#iptCTLCellPhone').prop('disabled', false);
        //要保人資料區解鎖
        $('#chkISAPL').prop('checked', false).prop('disabled', false);
        $('#divAPLData').find('input,select,button,textarea').each(function () { $(this).attr('disabled', false); });
        $('input[name="rdoAPLCustType"]').parent().parent().attr("style", "width:0%; background-color:white;border: 1px solid #cccccc;height:34px;");
        //強制險區鎖住
        $('#divManData').find('input,select,button,textarea').each(function () { $(this).attr('disabled', true); });
        //任意險區解鎖
        $('#divAnyData').find('input,select,button,textarea').each(function () { $(this).attr('disabled', false); });
        $('#iptAnySerialNo,#selCalCode,#selPAYWAY').prop('disabled', true);
        //執行強制車種事件帶入21 險種資料
        $('#selMANMVPZMTYPE').change();
        //檢查是否已投保過
        if (checkInsureExistResult.Success) {
            //MsgBox('錯誤', checkInsureExistResult.Msg, 'red');
            ctrlDiv(-1, "LOCK");            /*鎖住所有控制項*/
            $('#btnAssistClose').prop('disabled', false);
            ctrlDiv(-1, "OPEN");            /*開啟頁籤*/
            //廠代鎖住selZMAKE
        }
        else {
            //預設強制險日期同任意險日期
            if (assistData.compInsOrNot == "Y")
                $('#chkCCDATE').prop('checked', true).change();
            //清空[險種資料]保險費
            clrInsTBPremium();                                           
            //廠型所有[輸入框、按鈕]啟用
            $('#divZMAKE').find('input,select,button,textarea').each(function () { this.disabled = false; });
...
function ctrlInputCust() {
    var sBranchNo = $('#iptBranchNo').val();   //[單位別]
...
function ctrlZmakeWidth() {
    //輔助平台預設打開廠型選項
    var sCol = "eee";
    sCol = checkInsureExistResult != undefined && checkInsureExistResult.Success ? "eee" : "fff";  //當[下一步]按鈕出現時，則帶fff底色
...
ctrlIns.CalCode = function () {
    ...
    if (insuredperiod > 1 || AddYears(iptCCDATES.val(), 1) == iptCCDATEE.val()) {
        ...
        //輔助平台改為月繳時自動帶上並鎖住
        _div4.find('#selCalCode').val('SQ').attr('disabled', true);
    }
}
...
///*==[查詢類]_Start===========================*/
//查詢[輔助平台]資料
function getWebAssistData(sQuoteNo, sType) {
    consLogDate("--getWebQuotData_Start--");
    $('#iptQUOTENO').data('isSysPass', true);
    //--==查詢[報價單]資料================================================================--
    //consLogDate("--getWebQuotData_查詢資料");
    //assistData.handledID = "60-BD100";
    //assistData.handledName = "朱寶慧";
    //assistData.producerName = null;
    //assistData.producerRegNo = "A06D146929";
    //assistData.tradingCompCode = "20";
    //assistData.brandModel = "0101010";
    //assistData.makeYearMonth = "201907";
    //assistData.plateNo = "1638-AB";
    //assistData.capacityLimit = "5";
    //assistData.capacityUnit = "人";
    //assistData.cylinder = "1200.89";
    //assistData.orgIssueDate = "20191122";
    //assistData.compBeginDate = "2019112212";
    //assistData.compEndDate = "2020112212";
    //assistData.insuredCarType = "03";
    //assistData.feeGrade = "4";
    //assistData.compPremium = "1398";
    //assistData.insuredCardNo = null;
    //assistData.ownerId = "F227075867";
    //assistData.idType = "1";
    //assistData.birthDate = "19760501";
    //assistData.compRepName = "";
    //assistData.ownerName = "陳雅惠";
    //assistData.gender = "F";
    //assistData.city = "新北市";
    //assistData.zipCode = "106";
    //assistData.address = "新北市大安區敦化北路２６０號６樓";
    //assistData.email = "camay.huang@hotains.com.tw";
    //assistData.phone = "0988333555";
    //assistData.mobile = "0988333555";
    //assistData.txNo = "19102200032";
    //assistData.compInsOrNot = "N";
    //assistData.arbtyInsOrNot = "Y";
    //assistData.engineNo = "WDDGF4HB6DA782783"

    $('input[name="txNo"]').val(assistData.txNo);
    $('input[name="collectionType"]').val(assistData.collectionType == "1" ? "是" : assistData.collectionType == "2" ? "否" : "");
    $('input[name="arbtyInsOrNot"]').val(assistData.arbtyInsOrNot == "Y" ? "是" : assistData.arbtyInsOrNot == "N" ? "否" : "");
    var ds = null;
    checkInsureExistResult = getdata("/Assist/CheckInsureExist", { model: assistData });
    if (checkInsureExistResult.Success && checkInsureExistResult.Data[0]["QuotNo"] != null && checkInsureExistResult.Data[0]["QuotNo"] != "") {
        $("#iptQUOTENO").val(checkInsureExistResult.Data[0]["QuotNo"]);
        ds = getdata("/Quotation/GetQuot", { sQuotNo: checkInsureExistResult.Data[0]["QuotNo"] });      //報價單查詢
        if (checkInsureExistResult.Success && checkInsureExistResult.Data[0]["ZautclsMail"] != null && checkInsureExistResult.Data[0]["ZautclsMail"] != "") {
            MsgBox('', '任意險需經核保人員審核，請洽保險公司服務人員。', 'green');
        ...
        ds = getdata("/Assist/GetAssistData", { assistData: assistData });      //報價單查詢
    }
    //廠代初始化
    InitZMAKEList(assistData.brandModel, assistData.insuredCarType);
    ...
    if (dt != null && dt.length > 0) {
        ...
        var dtAgn = getdata("/Quotation/GetAgntInfo", { KeyWord: agentno, sSalesmanRegNo: dt[0].SalesmanRegNo });
        if (dtAgn.length > 0) {
            ...
            _div1.find('#iptCNTBRANCH').val(dtAgn[0].LifeNo);                                      //編號(壽通)
        ...
        _div1.find('#iptBranchNo').val(dt[0].BranchNo);                                     //單位別
        if (dt[0].LifeNo != null && dt[0].LifeNo != "")
        ...
        if (_div2.find('#selRELA').val() == '01') {
            ...
            ////----[要保人]資料--------------------------------------------------------
            var acusdt = ds.DT2;
            if (acusdt != undefined && acusdt.length > 0) {
...
function getWebQuotData(sQuoteNo, sType) {
    ...
    //--==查詢[報價單]資料================================================================--
    consLogDate("--getWebQuotData_查詢資料");
    var ds = getdata("/Quotation/GetQuot", { sQuotNo: sQuoteNo });      //報價單查詢
    var dt = ds.DT;
    if (dt.length > 0) {
        consLogDate("--getWebQuotData_[報價單]資料載入");
        if (dt.length === 1 && typeof (dt[0].MSG) != "undefined") { MsgBox('錯誤', dt[0].MSG, 'red'); return; }
        ...
        _div3.find('#selMANMVPZMTYPE').val(getObjToVal(dt[0].ForceCarType)).change();        //強制車種
        _div3.find('#selMVPZMTYPE').val(getObjToVal(dt[0].AnyCarType)).change();    //任意車種
        _div3.find('#selYRMANF').val(dt[0].MakeYear);                               //製造年
        _div3.find('#iptRESETPRICE').val(dt[0].CarPrice).blur();                    //重置價格
        _div3.find('#iptZREGNUM').val(dt[0].LicenseNo);                             //牌照
        _div3.find('#iptZCARRY').val(dt[0].Load);                                   //乘載(人/噸)
        _div3.find('#iptZCC').val(dt[0].Displacement).blur();                       //排氣量
...
function chkSub_CarData() {
    ...
    var sIssueYear = getObjToVal(_div3.find('#selIssueYear').val());          /*發照年*/
    var sMakeYear = getObjToVal(_div3.find('#selYRMANF').val().substr(0, 4));              /*製造年*/
...
function chkResetpriceRange() {
    ...
    var oldkey = parseInt(getObjToVal(_div3.find('#iptRESETPRICE').data('oldval')));// parseInt(getObjToVal($("#iptRESETPRICE").attr('oldval')).replace(/[$,]/g, ""));
...
function chkAny() {
    if ($('#chkAny').prop('checked')) {     //任意險資料
        ...
        if (mvpzmtype == '22') { mvpzmtype = '03'; }   //任意險沒有22車種，故將22改為03
...
function chkInsData(isCal) {
    ...
    if (zcvrtypelist.indexOf("28") != -1 || zcvrtypelist.indexOf("29") != -1 || zcvrtypelist.indexOf("30") != -1) {
        dtSource = getdata("/Quotation/ChkINS282930", { sINSDATA: inddatalist });
        if (dtSource.length > 0) {
...
function getLevel() {
    ...
    if ($('#chkMan').prop('checked') && iptForceSerialNo.val() == '') {
        ...
        if (result != null || typeof (result) != "undefined") {
            ...
            if ((result.Data.ForceMsgCode == "000") || (sCarCategory == "3" && result.Data.ForceMsgCode == "X99")) {
                if (sCarCategory != "3") {
                    iptForceSerialNo.val(result.Data.ForceSerialNo);      //強制序號
                    forcelevel = result.Data.ForceLevel;                        //強制等級
                    drunkfreq = result.Data.DrunkFreq;                          //酒次
        ...
        if (chkManFlag) {
            ...
            if (forcelevel != '' && forcelevel != selMANLEVEL.val() && selMANLEVEL.val() != '') {
                errobj = "selMANLEVEL";
                msg = "[強制等級]與保發取回的不一致(" + forcelevel + ")，請確認！";
            ...
            if (drunkfreq != '' && drunkfreq != iptDrunkFreq.val() && iptDrunkFreq.val() != '') {
                errobj = "iptDrunkFreq";
                msg = "[酒次]與保發取回的不一致(" + drunkfreq + ")，請確認！";
                ShowMsgInElm('#' + errobj, msg, true);
                errmsgMan += msg + "|";
                iptForceSerialNo.val('');     //清空[強制序號]
                $("#labManGetLevelMsg").text($("#labManGetLevelMsg").text() + ' ' + msg);
    ...
    if ($('#chkAny').prop('checked') && iptAnySerialNo.val() == '') {
        ...
        MOTInfo_Arr.CarType = ($('#selMVPZMTYPE').val() == "22") ? '03' : $('#selMVPZMTYPE').val(); //車種代碼
        ...
        if (result != null || typeof (result) != "undefined") {
            ...
            if (getObjToVal(result.Data.SysMsg) != "") { MsgBox('', result.Data.SysMsg, 'orange') }
            ...
            if (anymsgcode == "000") {
                ...
                $('#divQuoteDate').data("TaxiDutyFactor", getObjToVal(result.Data.TaxiDutyFactor));       //計程車責係
        ...
        if (chkAnyFlag) {
            ...
            if (bodyfactor != '' && bodyfactor != selBodyFactor.val() && selBodyFactor.val() != '') {
                ...
                msg = "[體係]與關貿不同(" + bodyfactor + ")，請檢附說明文件！";
                ShowMsgInElm('#' + errobj, msg, true);
                errmsgAny += msg + "|";
            ...
            if (dutyfactor != '' && dutyfactor != selDutyFactor.val() && selDutyFactor.val() != '') {
                ...
                msg = "[責係]與關貿不同(" + dutyfactor + ")，請檢附說明文件！";
                ShowMsgInElm('#' + errobj, msg, true);
                errmsgAny += msg + "|";
                iptAnySerialNo.val('');       //清空[任意序號]
                labAnyGetLevelMsg.text(labAnyGetLevelMsg.text() + ' ' + msg);
...
function getQuotData() {
    ...
    var retdata = {
        ...
        , MakeYear: assistData.makeYearMonth	                                //帶入輔助平台製造年6碼
...
//*==[列印類]_Start============================*/
//PDF列印
function PrintReport(dlLink, sModel, sPrintItems, writeLog) {
    ...
    printForm.append($("<input>").attr({ "type": "hidden", "name": "writeLog" }).val(writeLog));
