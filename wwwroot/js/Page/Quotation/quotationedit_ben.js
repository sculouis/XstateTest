...
$("#btnBenNew").on("click", function () {
    if (_divBen.find('#divBenInputData').is(":hidden")) {
        ...
        _divBen.find('#divBenUploadData').hide();
    } else {
...
$('#btnBenAdd').click(function (e) {
    ...
    var selSex = $('#selDriCLTSEX').val();
    var idSex = $('#iptDriMEMBSEL').val().substr(1, 1);
    if ((idSex == "1" && selSex != "1") || (idSex == "2" && selSex != "2")) {
        MsgBox('錯誤', 'ID與性別不符！', 'red');
        return;       
    }
    BlockUI('作業中，請稍後');
    setTimeout(function () {
...
$("#btnUpload").on("click", function () {
    if (_divBen.find('#divBenUploadData').is(":hidden")) {
        _divBen.find('#divBenUploadData').show(200);
        _divBen.find('#divBenInputData').hide();
    } else {
        _divBen.find('#divBenUploadData').hide(200);
    };
});
/******按鈕處理事件_End********************************/
