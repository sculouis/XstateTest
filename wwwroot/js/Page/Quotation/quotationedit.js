...
function InitEvent() {
    ...
    $('#iptCTLCustID,#iptCTLBirthday,#iptZREGNUM').on("change", function (e) {
        ...
        if (objNM == 'iptZREGNUM') {
            ctrlIns.UnlockProceedArea("ALL", this.id);   //20200312 ADD BY MICHAEL 若有異動[牌照]就要清空[舊保單號碼]
        }
    });
    //[付款方式]選取事件
    ...
    //[手機號碼]焦點離開事件
    $('#iptCTLCellPhone,#iptAPLCellPhone').on("blur", function (e) {
        removeMsgInElm('#' + e.target.id);   //訊息移除
        if (this.value == '') { return; };
    ...
    //[折舊率]選取事件
    _div4.find('#selDepRate').on("change", function (e) {
        if ($('#iptQUOTENO').data('isSysPass')) { return; };   //當[報價單編號]查詢帶入資料時/[清除]，不處理資料異動事件
        ctrlIns.UnlockProceedArea('1', this.id);             //檢查[是否為續保件]若是，則清空[上年度保單號碼]
    });    
    //[活動代碼]選取事件
...
function InitClick() {
    ...
    $('#btnUnPOLNUM,#btnUnOLDPOLNUM').on("click", function (e) {
        ...
        if (objBtnNM == '#btnUnPOLNUM') {
            ...
            getdata("/Quotation/UserLog_Info", { sActionDesc: "[上年度保單號碼]已清空！", sContent: "elementId:" + $(this).attr('id') + ", sPOLNUM:" + _divQ.find(objInpNM).val() });
            MsgBox('提醒', "[上年度保單號碼]已清空！", 'orange');
        } else {
            objInpNM = '#iptOLDPOLNUM';
            objBtnNM = '#btnQryOLDPOLNUM';
            //getdata("/Quotation/UserLog_Info", { sActionDesc: "[上年度強制證號]已清空！", sContent: "sPOLNUM:" + _divQ.find(objInpNM).val() });
    ...
    $('#btnNext2').on("click", function (e) {
        ...
        setTimeout(function () {
            ...
            var dt = getdata("/Quotation/ChkQuotExistsStep2", param);      //客戶資料查詢
            if (dt[0].col0 == '' || dt[0].col1 == '') {
    ...
    switch (ObjActNM) {
        ...
        case "chkCCDATE":       /*--==========[任意保險期間(同強制險)]==========--*/
            if (isChkCC) {
                ...
                if (isChkCC && iptCCDATES.val() != iptCCDATES.attr('oldval') && getObjToVal(iptCCDATES.attr('oldval')) != "") {
                    ctrlIns.UnlockProceedArea('1', ObjActNM);             //檢查[是否為續保件]若是，則清空[上年度保單號碼]
        ...
        case "iptFRDATES":      /*--==========[強制保期(起)]==========--*/
            ...
            if (isChkCC && iptFRDATES.val() != iptFRDATES.attr('oldval') && getObjToVal(iptFRDATES.attr('oldval')) != "") {
                ctrlIns.UnlockProceedArea('1', ObjActNM);                 //若[任意保險期間(同強制險)]，則清空[上年度保單號碼]
                if ($('#iptAnySerialNo').val() != "") { $('#iptAnySerialNo').val(''); };    //若有[任意險序號]，則清空
            };
            ctrlIns.UnlockProceedArea('2', ObjActNM);                     //檢查[是否為續保件]若是，則清空[上年度強制證號]
            break;
...
//控制_[續保作業]續保解鎖 參數:area=1:為[上年度保單號碼]區解鎖,2:為[上年度強制證號]區解鎖。elementId:執行此函數時的input id 
ctrlIns.UnlockProceedArea = function (area, elementId) {
    switch (area) {
        case "1":
            //若有[上年度保單號碼]則代表目前為[續保作業]，因為有異動日期，所以要清空。(這樣會變成純新件報價)
            if (_divQ.find('#iptPOLNUM').val() != '') {
                getdata("/Quotation/UserLog_Info", { sActionDesc: "[上年度保單號碼]已清空！", sContent: "elementId:" + elementId });
                _divQ.find('#btnUnPOLNUM').click();
            }
            break;
        case "2":
            //若有[上年度強制證號]則代表目前為[續保作業]，因為有異動日期，所以要清空。(這樣會變成純新件報價)
            if (_divQ.find('#iptOLDPOLNUM').val() != '') {
                getdata("/Quotation/UserLog_Info", { sActionDesc: "[上年度強制證號]已清空！", sContent: "elementId:" + elementId });
                _divQ.find('#btnUnOLDPOLNUM').click();
            }
            break;
        default:
            if (_divQ.find('#iptPOLNUM').val() != '') {
...
function chkSub_ProtectPeopleData() {
    ...
    //檢核_被保險人[手機號碼]
    if (isChk && _div2.find('#iptCTLCellPhone').val() != "") {
        if (!checkMobil(_div2.find('#iptCTLCellPhone').val())) {
            ctrlMsg('SF', '手機號碼格式錯誤！', '#iptCTLCellPhone'); isChk = false;
        };
    };
    //檢核_要保人[手機號碼]
    if (isChk && _div2.find('#iptAPLCellPhone').val() != "") {
        if (!checkMobil(_div2.find('#iptAPLCellPhone').val())) {
            ctrlMsg('SF', '手機號碼格式錯誤！', '#iptAPLCellPhone'); isChk = false;
        };
    };
    return isChk;
};
