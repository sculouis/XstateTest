//全域變數宣告
var _strInsGroupList = '06|16|1B|28|29|30|31|37|38|48|74|F5|76|9B|D1|D2|D3|A6';      /*需要做群組的險種清單*/
var _dtUserInsListMax = 20          /*[我的最愛]筆數*/
//初始化_元件事件(險種相關)
function InitEventIns() {
    ...
    $("#selZCAMPAN").on("change", function () {
        ...
        setTimeout(function () {
            ...
            if (key == "") {
                ...
                var dtSource = getdata("/Quotation/GetZCAMPAN", { sZCAMPAN: key, sMVPZMTYPE: $('#selMVPZMTYPE').val(), sCarAge: getCarAge(), sZCARRY: $('#iptZCARRY').val() });
                if (dtSource.length > 0) {
                    if (dtSource.length === 1 && typeof (dtSource[0].MSG) != "undefined") {
                        MsgBox('錯誤', dtSource[0].MSG, 'red'); selZCAMPAN.val(''); return;
                    };
                    //先將[活動方案]存入陣列裡
                    var dtZCAMPAN = [];
                    $.each(dtSource, function (i, item) {
                        dtZCAMPAN.push({
    ...
    $("#selZCVRTYPE").on("change", function () {
        ...
        setTimeout(function () {
                        ...
                        if (EXCESSList.indexOf(key) > -1) {
                            ...
                            $.each(data, function (idx, item) {
                                ...
                                if (item.EXCESSREQ == 'Y') {                    /*設定[自負額]下拉欄位為必填欄位*/
                                    $('#selEXCESS').removeAttr('disabled').attr('readonly', false);//.attr('required', 'required') /*20190130 UPD BY WS-MICHAEL 移除[自負額]下拉欄位為必填，因為自負額有可能是空值*/
                                }
                            ...
                            //---------險種[1B]特別處理程序_Start-------
                            $.each(data, function (idx, item) {
                                var sGListA = item.SUMINA;
                                var sGListE = item.EXCESS; //奇怪的符號
                                $TEXTA = $($.parseXML(sGListA)).find("TEXT");
                                $TEXTE = $($.parseXML(sGListE)).find("TEXT");
                                $VALUEE = $($.parseXML(sGListE)).find("VALUE");
                                var arr = [];
                                var ssuminA = "";
                                ...
                                for (let i = 0; i < $TEXTA.length; i++) {
                                    ...
                                    arr.push({
                                        SUMINA: $TEXTA[i].textContent, EXCESS: $TEXTE[i].textContent, VALUE: $VALUEE[i].textContent
                                    });
                                };
                                $("#divInsured").data("insArr", arr);           /*暫存陣列*/
                                setDDL_INS("#selSUMINA", ssuminA, dt);          /*設定[保額一]*/
                                $('#iptSUMINA').attr('style', 'display:none');  /*隱藏[保額一]輸入欄位*/
                                $('#selSUMINA').attr('style', 'display:');      /*顯示[保額一]下拉欄位*/
                                if (item.SUMINAREQ == 'Y') {                    /*設定[保額一]下拉欄位為必填欄位*/
                                    $('#selSUMINA').attr('required', 'required').removeAttr('disabled').attr('readonly', false);
                                };
                                setDDLRep("selSUMINA");                         /*[保額三]移除重複選項*/
                                if (item.EXCESSREQ == 'Y') {                    /*設定[自負額]下拉欄位為必填欄位*/
            ...
            //SDS回報_自負額同主險_規則
            var result = getdata("/Kernel/GetMainInsureTypeList", {
                sZCVRTYPE: key
            });
            ...
            if (result.Success) {
                if (result.Data.length > 0) {
                    ...
                    $.each(result.Data, function (key, value) {
                        ...
                        thisDT = $.grep(dt.rows().data(), function (x) {
                            return x.ZCVRTYPE == value.word.trim();
                        });

                        if (thisDT.length > 0) {
                            if (thisDT != "undefined" && thisDT != null) {
                                $('#selEXCESS').val(thisDT[0].EXCESS.toString());
                                return false;
                            }
                        } else {
                            //MsgBox('錯誤', '請選擇正確的險種', 'red');
                            //$('#selZCVRTYPE').val('');
                            //return false;
                        }
                    });
    ...
    $("#selSUMINA").on("change", function () {
        var sVal = $(this).val();
        if ($('#selZCVRTYPE').val() == '1B') {
            var arr = [];
            arr = $("#divInsured").data("insArr");
            arr = $.grep(arr, function (x) { return x.SUMINA == sVal; });
            var sEXCESS = "";
            arr.map(function (x) {
            ...
            setDDLRep("selEXCESS");
        } else if (_strInsGroupList.indexOf($('#selZCVRTYPE').val()) > -1) {       //為需要做群組的險種清單
            var arr = [];
            arr = $("#divInsured").data("insArr");
    ...
    $('#btnSetUserPro').on("click", function (e) {
        ...
        $('#TextBoxesGroup div').remove().end();
        if (divSet.is(":hidden")) {
            divSet.show(200);   /*開啟[自訂方案]設定*/
            $('#divSetUserPro').find('input,button').parent().hide();   /*所有[欄位/按鈕]隱藏*/
            var sZCAMPAN = $('#selZCAMPAN').val();
            var tmpInsr400 = _divQ.data("ZCAMPANLIST");
            var is400Item = $(tmpInsr400).find('VALUE').toArray().some(function (elm) { return $(elm).text() == sZCAMPAN });    /*AS400選單*/

            UserInsList = '<div class="UserPro" id="#maximum#"><div class="col-lg-3 col-md-6 col-sm-6 col-xs-12 input-group">\
                    <input type="text" id="iptProgramName#maximum#" maxlength="60" class="form-control upperCase hftofl" placeholder="新增方案">\
                    <span class="input-group-btn"><button id="btnSavePro#maximum#"class= "btn btn-default" type = "button" title = "依照目前[承保內容]，儲存至方案" onclick = "savePro(#maximum#, \'add\');" ><i class="fa fa-floppy-o"></i></button ></span >\
                    </div>\
                    </div>\
                    <div class="row"></div>\
                    <div><label style="color:#3a87ad; margin-bottom:0px;">您最多可新增' + _dtUserInsListMax + '組方案</label></div>';

            if (sZCAMPAN.length >= 1 && sZCAMPAN != '0') {
                //當[活動方案]內，有選擇"自訂方案"時   
                if (!is400Item) {
                    if (_dtUserInsList.length > 0) {
                        $.each(_dtUserInsList, function (i, item) {
                            sVal = getObjToVal(_dtUserInsList[i].VALUE);
                            if (sVal == sZCAMPAN) { sText = getObjToVal(_dtUserInsList[i].TEXT); }
                        });

                        $('#TextBoxesGroup').append('<div class="UserPro" id="' + sZCAMPAN + '"><div class="col-lg-3 col-md-6 col-sm-6 col-xs-12 input-group">' +
                            '<input type="text" id="iptProgramName' + sZCAMPAN + '" maxlength="60" class="form-control  upperCase hftofl" value=' + sText + '> ' +
                            '<span class="input-group-btn"><button id="btnSavePro' + sZCAMPAN + '"class= "btn btn-default" type = "button" title = "依照目前[承保內容]，儲存至方案" onclick = "savePro(' + sZCAMPAN + ', \'edit\');" ><i class="fa fa-floppy-o"></i></button ></span >' +
                            '<span class="input-group-btn"><button id="btnDelPro' + sZCAMPAN + '"class= "btn btn-default" type = "button" title = "刪除方案" onclick = "delPro(' + sZCAMPAN + ');" > <i class="fa fa-times"></i></button ></span > ' +
                            '</div>' +
                ...
                $.each(_dtUserInsList, function (i, item) {
                    sVal = getObjToVal(_dtUserInsList[i].VALUE);
                    sText = getObjToVal(_dtUserInsList[i].TEXT);

                    $('#TextBoxesGroup').append('<div class="UserPro" id="' + sVal + '"><div class="col-lg-3 col-md-6 col-sm-6 col-xs-12 input-group">' +
                        '<input type="text" id="iptProgramName' + sVal + '" maxlength="60" class="form-control  upperCase hftofl" value=' + sText + '> ' +
                        '<span class="input-group-btn"><button id="btnDelPro' + sVal + '"class= "btn btn-default" type = "button" title = "刪除方案" onclick = "delPro(' + sVal + ');" > <i class="fa fa-times"></i></button ></span > ' +
                        '</div>' +
                        '</div>' +
                        '<div class="row"></div>');
                });

                if (_dtUserInsList.length < _dtUserInsListMax) {
                    maximum = sVal + 1;
                } else {
                    MsgBox('提示', '已新增' + _dtUserInsListMax + '組方案！', 'green'); return;
                }
                $('#TextBoxesGroup').append(UserInsList.replace(/#maximum#/g, maximum));
            };
    ...
    //[新增自訂方案]按鈕
    $('#btnAddUserPro').on("click", function (e) {
        var maximum = 0;
        $('.UserPro').each(function () {
            var value = parseInt($(this).attr('id'));
            maximum = (value > maximum) ? value : maximum;
        });
        maximum += 1;

        $('#TextBoxesGroup').append('<div class="UserPro" id="' + maximum + '"><div class="col-lg-3 col-md-6 col-sm-6 col-xs-12 input-group">' +
            '<input type="text" id="iptProgramName' + maximum + '" maxlength="60" class="form-control upperCase hftofl">' +
            '<span class="input-group-btn"><button id="btnSavePro' + maximum + '"class= "btn btn-default" type = "button" title = "依照目前[承保內容]，儲存至方案" onclick = "savePro(' + maximum + ');" ><i class="fa fa-floppy-o"></i></button ></span >' +
            '<span class="input-group-btn"><button class= "btn btn-default" onclick="removeDiv(' + maximum + ');"><i class="fa fa-minus"></i></button></span >' +
            '</div>' +
            '</div><div class="row"></div>');
    });
    ////[儲存方案]按鈕
...
function CreatInsDT(dtSource) {
    ...
    $('#' + dtObjid).DataTable({
        ...
        columnDefs: [
        { visible: false, targets: [hideid, 10, 11, 12] },
        {
...
function insDel(zcvrtype) {
    ...
    var dt = $('#tblInsuranceList').DataTable();
    if (dt.rows().data().filter(function (x) { if (x.ZCVRTYPE == zcvrtype) { return true; } }).length > 0) {
        //20200210 DEL BY MICHAEL 不要提醒
        BlockUI('作業中，請稍後');
        setTimeout(function () {
            if (zcvrtype == '21') {
            ...
            //傷害險被保險人名冊，若投保傷害險(47.48.58險種)，則在新增傷險名冊後，再刪除險種，
            //則傷害險被保險人名冊也要刪除 modify by eric 2019/09/27           
            var oData = $('#tblBeneList').dataTable();
            $("#tblBeneList tbody > tr:contains('" + zcvrtype + "')").each(function (i) {
                oData.fnDeleteRow(zcvrtype);
            });
        ...
        if (thisDT.length > 0) {
            ...
            setTimeout(function () {
                ...
                if (EXCESSList.indexOf(zcvrtype) > -1) {
                    ...
                    //---------險種[1B]特別處理程序_Start-------
                    $('#selSUMINA').val(thisDT[0].SUMINA).change();
                    $('#selEXCESS').val(thisDT[0].EXCESS);
                    return;
                    //---------險種[1B]特別處理程序_End---------
                };
...
//儲存方案
function savePro(sProNo, sMode) {
    var sProNM = $('#iptProgramName' + sProNo).val();
    if (sProNM == "") { ctrlMsg("SF", "", "#iptProgramName" + sProNo); return; };
    var INS_Arr = $('#tblInsuranceList').DataTable().data().toArray();  //險種資料
    INS_Arr = INS_Arr.filter(function (x) { return x.ZCVRTYPE != '21' });
    if (INS_Arr.length == 0) { MsgBox('錯誤', "[承保內容]無任意險種，請重新選擇！", 'red'); return; };
    var userInsList = getdata("/Quotation/getUserInsList", {});
    //if (sMode == 'add') {
    //    if (userInsList.length >= 10) { MsgBox('提示', "已超過10組方案！", 'green'); return; }
    //}
    if (!chkDuplicate(sProNM, userInsList, sMode, sProNo)) {
        ConfirmBox('確認儲存?', '請確認是否將目前[承保內容]儲存至[' + sProNM + ']?', 'orange', function () {
            BlockUI('作業中，請稍後');
            setTimeout(function () {
                var param = { sProgramNo: sProNo, sProgramName: sProNM, sProgCode: "QuotationEdit", INS_Data: INS_Arr };    //寫入參數
...
//刪除方案
function delPro(sProNo) {
    var sProNM = $('#iptProgramName' + sProNo).val();
    ConfirmBox('確認刪除?', '請確認是否將目前方案[' + sProNM + ']進行刪除?', 'orange', function () {
        BlockUI('作業中，請稍後');
        setTimeout(function () {
            _dtUserInsList = getdata("/Quotation/UserInsSettingDel", { sProgramNo: sProNo });
            setDDLZCAMPAN(_dtUserInsList);              //設定[活動方案/自訂方案]下拉選單內容
            $('#selZCAMPAN').val('');       //清空[活動方案]
            $('#divSetUserPro').hide(200);  //關閉[自訂方案]
            //removeDiv(sProNo);
...
//檢查輸入方案是否重複
function chkDuplicate(sProNM, userInsList, sMode, sProNo) {
    var isDuplicate = false;
    if (userInsList.length > 0) {
        if (sMode == 'add') {
            $.each(userInsList, function (i, item) {
                if (sProNM == getObjToVal(userInsList[i].TEXT)) {
                    MsgBox('提示', "活動方案已存在，請重新輸入！", 'green');
                    isDuplicate = true;
                    return false;
                }
            });
            ...
            $.each(userInsList, function (i, item) {
                if (sProNM == getObjToVal(userInsList[i].TEXT) && sProNo != getObjToVal(userInsList[i].VALUE)) {
                    MsgBox('提示', "活動方案已存在，請重新輸入！", 'green');
                    isDuplicate = true;
                    return false;
                }
